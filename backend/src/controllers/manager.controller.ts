...
export const getManagerById = AsyncHandler(async(req:Request,res:Response)=>{
    const {authId} = req.params;
    const [manager] = await sql`SELECT * FROM "Manager" WHERE "authId" = ${authId}`;
    if(!manager){
...
export const updatemanager = AsyncHandler(async(req:AuthenticatedRequest,res:Response)=>{
    ...
    const updatedManager = await sql`UPDATE "Manager" SET name = ${name}, email = ${email}, "phoneNumber" = ${phoneNumber} WHERE "authId" = ${authId}`;
    if (!updatedManager) {
        throw new ApiError(404,"Manager not found");
    };
...
export const createManager = AsyncHandler(async(req:Request,res:Response)=>{
    ...
    const [manager] = await sql`INSERT INTO "Manager" (name,email,"phoneNumber","authId") VALUES (${name},${email},${phoneNumber},${authId})`;
    return res.status(201).json({message:"Manager created successfully",manager});
})

export const getManagerProperties = AsyncHandler(async(req:Request,res:Response)=>{
    const {authId} = req.params;
