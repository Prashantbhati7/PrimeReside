...
export const getProperties = async (req, res) => {
  ...
  const properties = propertiesResult.map((p) => ({
    ...p,
    amenities: Array.isArray(p.amenities)
      ? p.amenities
      : typeof p.amenities === "string"
      ? p.amenities.replace(/[{}]/g, "").split(",")
      : [],
    highlights: Array.isArray(p.highlights)
      ? p.highlights
      : typeof p.highlights === "string"
      ? p.highlights.replace(/[{}]/g, "").split(",")
      : [],
  }));

  return res
    .status(200)
    .json({ message: "properties fetched successfully", properties });
};
...
export const getPropertyById = AsyncHandler(async (req, res) => {
  ...
  const property = [
    {
      ...propertyResult[0],
      amenities: Array.isArray(propertyResult[0].amenities)
        ? propertyResult[0].amenities
        : typeof propertyResult[0].amenities === "string"
        ? propertyResult[0].amenities.replace(/[{}]/g, "").split(",")
        : [],
      highlights: Array.isArray(propertyResult[0].highlights)
        ? propertyResult[0].highlights
        : typeof propertyResult[0].highlights === "string"
        ? propertyResult[0].highlights.replace(/[{}]/g, "").split(",")
        : [],
    },
  ];
...
export const getPropertyLeases = AsyncHandler(async (req, res) => {
  const { id } = req.params;
  ...
  return res
    .status(200)
    .json({ message: "leases fetched successfully", leases });
});
