...
export const getALLTenants = AsyncHandler(async (req, res) => {
  ...
  return res
    .status(200)
    .json({ message: "tenants fetched successfully", tenants });
});
...
export const getCurrResidence = AsyncHandler(async (req, res) => {
  const { authId } = req.params;
...
export const ToggleFavorite = AsyncHandler(async (req, res) => {
  ...
  if (existing) {
    ...
    return res
      .status(200)
      .json({ message: "Property removed from favorites", isFavorite: false });
  } else {
    ...
    return res
      .status(200)
      .json({ message: "Property added to favorites", isFavorite: true });
  }
});
